...
// This function produces Nb(Nr+1) round keys. The round keys are used in each round to aesDecrypt the states.
...
void AES_CBC_decrypt_buffer(uint8_t* output, uint8_t* input, uint32_t length, const uint8_t* key, const uint8_t* iv)
{
  ...
  // If iv is passed as 0, we continue to aesEncrypt without re-setting the Iv
  if (iv != 0)
  {
    Iv = (uint8_t*)iv;
  }
